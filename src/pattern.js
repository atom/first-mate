...
export default class Pattern {
  constructor (grammar, registry, options = {}) {
    ...
    if (match) {
      ...
      this.captures = beginCaptures || captures
      const endPattern = this.grammar.createPattern({match: end, captures: endCaptures || captures, popRule: true})
  ...
  resolveScopeName (scopeName, line, captureIndices) {
    return scopeName.replace(AllCustomCaptureIndicesRegex, function (match, index, commandIndex, command) {
      const capture = captureIndices[parseInt(index || commandIndex)]
      if (capture) {
  ...
  tagsForCaptureIndices (line, currentCaptureIndices, allCaptureIndices, stack) {
    let parentCaptureScope
    const parentCapture = currentCaptureIndices.shift()

    const tags = []
    const scope = this.captures[parentCapture.index] ? this.captures[parentCapture.index].name : undefined
    if (scope) {
    ...
    const captureRule = this.captures[parentCapture.index] ? this.captures[parentCapture.index].rule : undefined
    if (captureRule) {
      const captureTags = this.tagsForCaptureRule(captureRule, line, parentCapture.start, parentCapture.end, stack)
      ...
      while (currentCaptureIndices.length && currentCaptureIndices[0].start < parentCapture.end) {
        ...
        const captureTags = this.tagsForCaptureIndices(line, currentCaptureIndices, allCaptureIndices, stack)
        tags.push(...captureTags)
